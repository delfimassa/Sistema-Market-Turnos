...
const categorias = [
  { op: "Alimentos", ph: " Ej: Marca:dogui Razas:Pequenas Cantidad:5kg" },
  { op: "Articulos de Cuidado", ph: "Ej: Shampoo para pulgas 250 ml." },
  { op: "Accesorios", ph: "Ej: Collar perro raza pequena" },
  { op: "Ropa", ph: "Ej: Buzo super man razas pequenas" },
];
export default function FormAdmin({
  ...
  const handleSubmit = async (event) => {
    ...
    if (!modificacion) {
      ...
      setProducto(productos.find((prod) => prod._id === producto._id));
      const solicitud = await fetch(
      ...
      if (solicitud.ok) {
        setProductos([
          ...productos.filter((t) => t._id !== producto._id),
          respuesta,
        ]);
        setBanderaProductos1(true);
